...
function removeTaskFromLocalStorage(taskToDelete) {
    ...
    tasks = task.filter(task => task.title !== taskToDelete.title || task.date !== taskToDelete.date);
    localStorage.setItem("tasks", JSON.stringify(tasks));
}
...
function updateLocalStorage() {
    ...
    document.querySelectorAll('.task-item').forEach(taskItem => {
        const task = {
            ...
            date: taskItem.querySelector('.task-date-time').textContent.split(", ")[0].replace("Дата: ", ""),
            time: taskItem.querySelector('.task-date-time').textContent.split(", ")[1].replace("Время: ", ""),
